...
// === 지도 스타일 패널(범례 바로 아래 / 범례 폭 동기화 / 접기·펼치기 / MapTiler 느낌) ===
(function addMapStylePanelUnderLegend(){
  // ✅ 같은 스코프에서 mlMap을 잡아야 ReferenceError가 안 납니다.
  const _mlMap = (typeof mlMap !== "undefined") ? mlMap : null;
  if (!_mlMap || typeof _mlMap.setStyle !== "function") return;
  ...
  // KEY는 위에서 이미 const KEY = "..."; 로 존재한다고 가정(현재 구조 그대로)
  const styleUrl = (slug) => `https://api.maptiler.com/maps/${slug}/style.json?key=${KEY}`;
  ...
  if (!document.getElementById(cssId)){
    ...
    st.textContent = `
      ...
      .mt-style-title{font-weight:800;letter-spacing:-.2px;}
      .mt-style-toggle{
        width:28px;height:28px;border-radius:10px;cursor:pointer;
        border:1px solid rgba(255,255,255,.16);
        background: rgba(15,16,17,.65);
        color:#e9eef7;
      }
      .mt-style-body{
        display:grid;
      ...
      .mt-style-btn.is-active{
        border-color: rgba(162,222,204,.95);
  ...
  const storageKey = "FRONTIER_MT_STYLE_PANEL_COLLAPSED";
  ...
  ctrl.onAdd = function(){
    root = L.DomUtil.create("div", "leaflet-control mt-style-panel");

    root.innerHTML = `
      <div class="mt-style-hd">
        <div class="mt-style-title">지도 스타일</div>
        <button type="button" class="mt-style-toggle" aria-label="toggle">−</button>
      </div>
      <div class="mt-style-body"></div>
    `;
    ...
    const toggle = root.querySelector(".mt-style-toggle");

    // 버튼 생성
    body.innerHTML = STYLES.map(s => `
      <button type="button" class="mt-style-btn" data-slug="${s.slug}">
        ${s.label}
      </button>
    `).join("");

    const setActive = () => {
      root.querySelectorAll(".mt-style-btn").forEach(btn => {
        const on = (btn.getAttribute("data-slug") === currentSlug);
    ...
    const applyCollapse = () => {
      ...
      // 접히면 높이가 줄어들고, 범례 아래에 있으면 자동으로 위로 딸려 올라갑니다.
      syncUnderLegend();
    };
    ...
    const switchStyle = (slug) => {
      ...
      try{
        _mlMap.setStyle(styleUrl(slug));

        // 스타일 변경 후 한글 라벨 재적용(한 번만)
        if (patchKo && typeof _mlMap.once === "function"){
          let ran = false;
          const onceRun = () => { if (ran) return; ran = true; patchKo(_mlMap); };
          _mlMap.once("idle", onceRun);
          _mlMap.once("styledata", onceRun);
        } else if (patchKo) {
          setTimeout(() => patchKo(_mlMap), 1200);
        }
      }catch(e){
    ...
    body.querySelectorAll(".mt-style-btn").forEach(btn => {
  ...
  function syncUnderLegend(){
    try{
      ...
      // Leaflet 컨트롤은 corner의 "직계 자식"으로 쌓입니다.
...
if (!root._legendRO && typeof ResizeObserver === "function"){
  ...
  setTimeout(syncUnderLegend, 200);
  setTimeout(syncUnderLegend, 800);
  setTimeout(syncUnderLegend, 2000);
  window.addEventListener("resize", syncUnderLegend);

  try{
    const corner = map.getContainer().querySelector(".leaflet-top.leaflet-left");
    if (corner && typeof MutationObserver === "function"){
      const mo = new MutationObserver(() => syncUnderLegend());
      mo.observe(corner, { childList: true, subtree: false });
    }
  }catch(e){}
})();     
