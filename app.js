...
// }).addTo(map);
    ...
    markers.on("clusterclick", (e) => {
      ...
      map.fitBounds(bb, { padding:[90,90], maxZoom: 18, animate:true, duration:1.0, easeLinearity:0.25 });
      }else if (e.layer && e.layer.getLatLng){
        const ll = e.layer.getLatLng();
      map.setView(ll, Math.min(map.getZoom() + 2, 18), { animate:true, duration:1.0, easeLinearity:0.25 });
      }
    });
